...
include "acsincludes/login_access.php";

// Check if the logged-in user is a Team Leader
if ($user_position == "Team Leader" && $EMP_TEAMLEADER == "YES") {
    // If the user is a Team Leader, retrieve their department
    $teamLeaderDepartment = $user_department;
} elseif ($user_position == "General Manager") {
    // If the user is a General Manager, set a flag to indicate full access
    $generalManagerAccess = true;
} else {
    // For other users, redirect them to an unauthorized page or display an error message
    header("Location: unauthorized.php"); // Redirect to an unauthorized page
    exit; // Stop further execution of the script
}

// If the user is a Team Leader or General Manager, construct the SQL query
if (isset($teamLeaderDepartment)) {
    // Team Leader can see only their department's requests
    $sql_timeoff = "SELECT `ID`, `EMP_ID`, `EMPLOYEE_NAME`, `DEPARTMENT`, `FROM_DATE`, `TO_DATE`, `FULL_HALF_DAY`, `FISRT_SECOND_OFF`, `MESSAGE`, `STATUS_LEAVE` FROM `time_off_tracking` WHERE STATUS_LEAVE = 'Pending' AND `FROM_DATE` >= '$currentDate' AND `DEPARTMENT` = '$teamLeaderDepartment' ORDER BY `FROM_DATE` DESC";
} elseif (isset($generalManagerAccess)) {
    // General Manager can see all requests
    $sql_timeoff = "SELECT `ID`, `EMP_ID`, `EMPLOYEE_NAME`, `DEPARTMENT`, `FROM_DATE`, `TO_DATE`, `FULL_HALF_DAY`, `FISRT_SECOND_OFF`, `MESSAGE`, `STATUS_LEAVE` FROM `time_off_tracking` WHERE STATUS_LEAVE = 'Pending' AND `FROM_DATE` >= '$currentDate' ORDER BY `FROM_DATE` DESC";
}
...
if (isset($_POST['all_button'])) {
    $sql_timeoff = "SELECT `ID`, `EMP_ID`, `EMPLOYEE_NAME`,  `DEPARTMENT`,`FROM_DATE`, `TO_DATE`, `FULL_HALF_DAY`, `FISRT_SECOND_OFF`, `MESSAGE`, `STATUS_LEAVE` FROM `time_off_tracking` WHERE `FROM_DATE` >= '$currentDate' ORDER BY `FROM_DATE` DESC";
    

} elseif (isset($_POST['pending_button'])) {
    $sql_timeoff = "SELECT `ID`, `EMP_ID`, `EMPLOYEE_NAME`, `DEPARTMENT`, `FROM_DATE`, `TO_DATE`, `FULL_HALF_DAY`, `FISRT_SECOND_OFF`, `MESSAGE`, `STATUS_LEAVE` FROM `time_off_tracking` WHERE STATUS_LEAVE = 'Pending' AND `FROM_DATE` >= '$currentDate' ORDER BY `FROM_DATE` DESC";
} elseif (isset($_POST['rejected_button'])) {
    $sql_timeoff = "SELECT `ID`, `EMP_ID`, `EMPLOYEE_NAME`,  `DEPARTMENT`,`FROM_DATE`, `TO_DATE`, `FULL_HALF_DAY`, `FISRT_SECOND_OFF`, `MESSAGE`, `STATUS_LEAVE` FROM `time_off_tracking` WHERE STATUS_LEAVE = 'Rejected'AND `FROM_DATE` >= '$currentDate' ORDER BY `FROM_DATE` DESC";
}  elseif (isset($_POST['approved_button'])) {
    $sql_timeoff = "SELECT `ID`, `EMP_ID`, `EMPLOYEE_NAME`, `DEPARTMENT`, `FROM_DATE`, `TO_DATE`, `FULL_HALF_DAY`, `FISRT_SECOND_OFF`, `MESSAGE`, `STATUS_LEAVE` FROM `time_off_tracking` WHERE STATUS_LEAVE = 'Approved'AND `FROM_DATE` >= '$currentDate' ORDER BY `FROM_DATE` DESC";
} else {
    // Default query to fetch all records
    $sql_timeoff = "SELECT `ID`, `EMP_ID`, `EMPLOYEE_NAME`,  `DEPARTMENT`,`FROM_DATE`, `TO_DATE`, `FULL_HALF_DAY`, `FISRT_SECOND_OFF`, `MESSAGE`, `STATUS_LEAVE` FROM `time_off_tracking` WHERE STATUS_LEAVE = 'Pending'";
}
...
<head>
    ...
    <link rel="stylesheet" href="Acscss/time_offf.css"/>  
    <style>
                ...
                <div class="icon_pro_bell">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu Profile_dropdown">
                            ...
                            <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item" href="acsincludes/logout.php">Sign out</a></li>
                        </ul>
                    </li>
                </div>
            </ul>
...
<!-- ---------------------gjgufig-------------------- -->
    <div class=" container_time_off">
     ...
     <div class="col-12 search_emp_row">
            <form action="time_offphp.php" class="form-inline my-2 my-lg-0">
...
<div class="tabless">
    ...
    <div class="table_timeoff">
        <form  action="controllers/form_control.php" method="post">
